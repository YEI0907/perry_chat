...
class ThreadSafeFaiss(ThreadSafeObject[FAISS]):
    """
    ...
    async def async_save(self, path: str, create_path: bool = True):
        """
        异步保存向量存储到指定路径。
...
class _FaissPool(CachePool[ThreadSafeFaiss]):
    """
    ...
    def new_vector_store(
        ...
        参数：
        embed_model (str): 嵌入模型名称，默认值为 None
    ...
    async def async_save_vector_store(self, kb_name: str, path: str=None):
        ...
        """
        if cache := self.get(kb_name):
            return await cache.async_save(path)
        return None
    ...
    async def load_vector_store(
        ...
        async with self.async_atomic:
            ...
            if cache is None:
                ...
                async with item.async_acquire(msg="初始化"):
                    # self.atomic.release()
